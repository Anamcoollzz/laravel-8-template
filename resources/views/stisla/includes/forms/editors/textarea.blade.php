...
@if (config('app.template') === 'stisla')
  <div class="form-group">
    ...
    <textarea {!! implode(' ', $props) !!}
      class="form-control {{ $errors->has($name ?? $id) ? 'is-invalid' : '' }}">{{ old($name ?? $id) ?? ($value ?? ($d[$name ?? $id] ?? '')) }}</textarea>
    @if ($hint ?? false)
...
@else
  <div class="form-group {{ $errors->has($name ?? $id) ? 'has-error' : '' }}">
    ...
    <textarea @if ($placeholder ?? false) placeholder="{{ $placeholder }}" @endif name="{{ $name ?? $id }}"
      @isset($id) id="{{ $id }}" @endisset @isset($disabled) disabled @endisset @if ($required ?? false) required
