@if (is_stisla_template())
  @include('stisla.includes.forms.selects.select', ['select2'=>true, 'multiple'=>$multiple??false])
...
@else
  ...
  <div class="form-group">
    ...
    <select @if ($multiple ?? false) multiple @endif name="{{ $name ?? $id }}" @isset($id) id="{{ $id }}" @endisset
      class="form-control {{ $multiple ?? false ? 'select2multiple' : 'select2' }}">
      @foreach ($options as $value => $text)
        <option @if ($selected ?? false === $value) selected @endif value="{{ $value }}">{{ $text }}</option>
      @endforeach
    </select>
  </div>
