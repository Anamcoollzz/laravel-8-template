...
class MakeCrudCommand extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($this->json->columns as $column) {
            ...
            else if (in_array($column->type, ['date', 'tinyInteger', 'text', 'double', 'integer', 'boolean'])) {
            ...
            if (isset($column->form)) {
                ...
                switch ($column->form->type) {
                  ...
                  @include('stisla.includes.forms.inputs.input', ['required'=>true, 'type'=>'time', 'id'=>'$column->name', 'name'=>'$column->name', 'label'=>__('" . $label . "')])
                    ...
                    case 'colorpicker':
                        $FORM .= "\t\t\t\t<div class=\"col-md-6\">
                  @include('stisla.includes.forms.colorpicker', ['required'=>true, 'type'=>'text', 'id'=>'$column->name', 'name'=>'$column->name', 'label'=>__('" . $label . "')])
                </div>\n\n";
                        break;
                    case 'date':
                        $FORM .= "\t\t\t\t<div class=\"col-md-6\">
                  @include('stisla.includes.forms.inputs.input', ['required'=>true, 'type'=>'date', 'id'=>'$column->name', 'name'=>'$column->name', 'label'=>__('" . $label . "')])
                    ...
                    case 'textarea':
                        $FORM .= "\t\t\t\t<div class=\"col-md-6\">
                  @include('stisla.includes.forms.textarea', ['required'=>true, 'id'=>'$column->name', 'name'=>'$column->name', 'label'=>__('" . $label . "')])
                </div>\n\n";
                        break;
                    case 'select2':
                  ...
                  @include('stisla.includes.forms.selects.select', ['required'=>true, 'id'=>'$column->name', 'name'=>'$column->name', 'label'=>__('" . $label . "'), 'options'=>" . $options . "])
                    ...
                    case 'radio':
                        ...
                        $options = json_encode($newOptions);
                        $FORM .= "\t\t\t\t<div class=\"col-md-6\">
                  @include('stisla.includes.forms.radio-toggle', ['required'=>true, 'id'=>'$column->name', 'name'=>'$column->name', 'label'=>__('" . $label . "'), 'options'=>" . $options . "])
                </div>\n\n";
                        break;
                }
            }
        ...
        $controllerFile = str_replace('FOLDERVIEW', $routeName = Str::kebab($modelName) . 's', $controllerFile);
        ...
        $viewIndexFile = str_replace('ROUTE', ($routeName = Str::kebab($modelName) . 's'), $viewIndexFile);
        ...
        $folder = base_path('resources/views/') . $routeName;
        // dd($folder);
        ...
        $exportExcelFile = str_replace('FOLDERVIEW', $routeName, $exportExcelFile);
        ...
        $this->info('Route::get(\'' . ($modelNameSlug = Str::kebab($modelName)) . 's/import-excel-example\', [\App\Http\Controllers\\' . $modelName . 'Controller::class, \'importExcelExample\'])->name(\'' . $modelNameSlug . 's.import-excel-example\');');
